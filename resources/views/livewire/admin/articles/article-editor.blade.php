...
<div x-data="articleEditor()" class="flex flex-col lg:flex-row gap-6 relative">
    ...
    <div class="w-full lg:w-4/12 flex flex-col gap-6">
        ...
        <template x-teleport="body">
            <div id="previewWindow" class="win-window shadow-2xl shadow-black/80 overflow-hidden resize bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg flex flex-col" style="display: none; position: fixed; z-index: 99999; top: 10vh; left: calc(50% - 390px); width: 780px; height: 85vh; max-width: 90vw; max-height: 90vh;">

                <!-- Window Title Bar -->
                <div class="win-titlebar flex items-center justify-between bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 pl-4 cursor-move select-none w-full h-8">
                    <span class="text-xs font-semibold text-gray-700 dark:text-gray-200 flex items-center gap-1.5 truncate max-w-[60%]">
                    ...
                    <div class="flex items-center">
                        ...
                        <button type="button" onclick="openFullPreview()" class="w-10 h-8 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-colors" title="Open in New Tab">
                            <svg viewBox="0 0 24 24" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
                            </svg>
                        </button>
                        <div class="h-4 w-px bg-gray-300 dark:bg-gray-600 my-auto"></div>
                        <div class="win-controls flex items-center">
                            <button onclick="WinManager.minimize('previewWindow')" class="w-11 h-8 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-colors" title="Minimize">
                                <svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 10 10"><path d="M0 5h10v1H0z"/></svg>
                            </button>
                            <button onclick="WinManager.maximize('previewWindow')" class="w-11 h-8 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-colors" title="Maximize">
                                <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" stroke-width="1" viewBox="0 0 10 10"><rect x="1.5" y="1.5" width="7" height="7"/></svg>
                            </button>
                            <button onclick="WinManager.close('previewWindow')" class="w-11 h-8 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-red-600 hover:text-white transition-colors animate-none" title="Close">
                                <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 10 10"><path d="M1 1l8 8M9 1L1 9"/></svg>
                            </button>
                        </div>
                    </div>
                </div>
...
(function() {
    ...
    // Function to update the Live Preview content
    window.updatePreview = function(html) {
        ...
        if (emptyState) {
            emptyState.style.display = html.trim() ? 'none' : 'block';
        }
    };
    ...
    window.openFullPreview = function() {
        ...
        if (!winBody) return;
        const html = winBody.innerHTML;
        const newWindow = window.open();
        newWindow.document.write(`
            <html>
            <head>
                <title>Preview Artikel - Narasumber Hukum</title>
                <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
                <style>
                    body { background: #f3f4f6; color: #111827; margin: 0; padding: 20px; font-family: 'Inter', sans-serif; }
                    .preview-container { background: white; max-width: 800px; margin: 0 auto; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.1); }
                </style>
            </head>
            <body>
    ...
    window.openPreviewInNewTab = window.openFullPreview; // alias
